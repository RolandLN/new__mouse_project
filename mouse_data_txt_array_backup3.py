...
def main(usr, txt_file, raw_data):
    """将raw__data原始数据提取出各项特征到txt_file中"""
    with open(raw_data, "r") as file:
        while True:
            ...
            for line in file:
                ...
                # print(line_list)
                if len(line_list) < 4:  # 判断最后一行是否完整，确保提取完整事件名
                    txt_file.close()
                    sys.exit()
                    # break
                ...
                if Eigenvalue_dict["i"] >= 1000:    # 每一千个数据为一组提取特征
                    ...
                    txt_file.write(
                        usr + "," + str(Eigenvalue_dict["NONE_number"]) + "," + str(Eigenvalue_dict["L_number"]) + ","
...
if __name__ == "main":
    user_number = "5"
    txt_file_place = open("D:/pyfile/project/biyelunwen/data_txt.txt", "a")
    raw_data_place= "E:/论文/data/2018鼠标数据集/Bvegetas_Retlati'ousabai/mouse-2018-12-13-00-34-46.txt"
    main(user_number, txt_file_place, raw_data_place)
